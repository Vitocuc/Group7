...
static void lpspi_flush_txfifo(NXPS32K358LPSPIState *s)
{
    ...
    // Check PINCFG for loopback mode (PINCFG=1 enables internal loopback)
    uint8_t pincfg = (s->lpspi_cfgr1 >> LPSPI_CFGR1_PINCFG_SHIFT) & 0x3;
    if (pincfg == 1)
    {
        // Internal loopback mode - RX data equals TX data
        rx_data = tx_data;
        DB_PRINT("SPI transfer (loopback): tx=0x%08x -> rx=0x%08x\n", tx_data, rx_data);
    }
    else
    {
        // Normal SPI transfer to external device
...
static void nxps32k358_lpspi_write(void *opaque, hwaddr addr, uint64_t val64, unsigned int size)
{
    ...
    case S32K_LPSPI_CR:
        ...
        s->lpspi_cr = value & ~(LPSPI_CR_RST | LPSPI_CR_RTF | LPSPI_CR_RRF);
        break;
    ...
    case S32K_LPSPI_TDR:
        ...
        if (s->lpspi_cr & LPSPI_CR_MEN)
        {
            ...
            for (int i = 0; i < bytes_per_frame; i++)
            {
                ...
                {
                    byte = (val64 >> ((bytes_per_frame - 1 - i) * 8)) & 0xFF;
                }
...
static void nxps32k358_lpspi_init(Object *dev)
{
    ...
    s->clk = qdev_init_clock_in(DEVICE(s), "clk", NULL, NULL, 0);
